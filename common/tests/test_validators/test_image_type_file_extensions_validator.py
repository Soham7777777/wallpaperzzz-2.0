...
class TestImageTypeFileExtensionsValidator(SimpleTestCase):
    ...
    image_type_file_extension_validator = ImageTypeFileExtensionsValidator(ImageType.PNG)
    ...
    def test_validation_for_invalid_extension(self) -> None:
        ...
        with self.assertRaises(ValidationError) as err:
            self.image_type_file_extension_validator(self.mock_image_field_file)
        self.assertEqual(err.exception.args[0], "Ensure that the image file has an extension from the supported file extensions: %(extensions)s for %(image_type)s type image.")
        self.assertEqual(err.exception.code, 'invalid_image_extension')
        self.assertEqual(err.exception.params, {'extensions': get_file_extensions_for_image_type(ImageType.PNG), 'image_type': ImageType.PNG})
